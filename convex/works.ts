...
export const listDefaults = query({
  ...
  handler: async (ctx, args) => {
    ...
    const rows = await ctx.db
      .query("worksDefaults")
      .withIndex("by_owner_and_stage", (q) => q.eq("ownerUserId", userId))
      .collect();
...
export const upsertDefault = mutation({
  args: {
    id: v.optional(v.id("worksDefaults")),
    stageType: stageUnion,
    name: v.string(),
    consumptionPerUnit: v.number(),
    purchasePrice: v.number(),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.id) {
      ...
      await ctx.db.insert("worksDefaults", {
        ownerUserId: userId,
        stageType: args.stageType,
        name: args.name,
        consumptionPerUnit: args.consumptionPerUnit,
...
export const upsertProjectWork = mutation({
  args: {
    id: v.optional(v.id("projectWorks")),
    projectId: v.id("projects"),
    stageType: stageUnion,
    name: v.string(),
    consumptionPerUnit: v.number(),
  ...
  handler: async (ctx, args) => {
    if (args.id) {
      ...
      await ctx.db.insert("projectWorks", {
