...
export default function OpeningMaterialsPage() {
  ...
  const [form, setForm] = useState({ name: "", consumptionPerUnit: "", purchasePrice: "", sellPrice: "", unit: "" });
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white border rounded-lg p-4 mb-4">
        <div className="grid grid-cols-5 gap-2 mb-2">
          ...
          <input className="border rounded px-2 py-1" placeholder="Расход/м² проёма" value={form.consumptionPerUnit} onChange={e=>setForm({...form, consumptionPerUnit:e.target.value})} />
        ...
        <button className="bg-emerald-600 text-white px-3 py-1 rounded" onClick={()=>{
          if(!form.name) return;
          upsert({ openingType: active, name: form.name, consumptionPerUnit: Number(form.consumptionPerUnit||0), purchasePrice: Number(form.purchasePrice||0), sellPrice: Number(form.sellPrice||0), unit: form.unit||undefined });
          setForm({ name:"", consumptionPerUnit:"", purchasePrice:"", sellPrice:"", unit:"" });
        }}>Добавить</button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              ...
              <th className="text-left p-2">Расход/м²</th>
